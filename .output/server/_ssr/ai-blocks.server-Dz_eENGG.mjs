...
//#region node_modules/.nitro/vite/services/ssr/assets/ai-blocks.server-Dz_eENGG.js
/**
...
async function readPageForAi(url) {
	let parsedUrl;
	try {
	...
	const html = (await res.text()).slice(0, 3e6);
	const finalUrl = res.url || parsedUrl.href;
	const { parseHTML } = await import("../_libs/linkedom.mjs").then((n) => n.t);
	...
	const meta = (sel) => document.querySelector(sel)?.getAttribute("content")?.replace(/\s+/g, " ").trim() || "";
	const title = meta("meta[property='og:title']") || (document.querySelector("title")?.textContent || "").trim() || (document.querySelector("h1")?.textContent || "").trim();
	...
	const parts = [];
	main?.querySelectorAll("h1,h2,h3,h4,p,li,td,th,blockquote,figcaption,a.btn,button").forEach((node) => {
	...
	if (text.replace(/\s/g, "").length < 40) return {
		ok: false,
		error: "محتوای متنی قابل خواندنی در این صفحه پیدا نشد."
	};
	return {
		ok: true,
		title,
...
function toBlocks(raw, images) {
	...
	for (const item of list.slice(0, 60)) {
		...
		const str = (v, max = 6e3) => typeof v === "string" ? v.slice(0, max) : "";
		let props = {};
		if (type === "hero") props = {
			...
			const items = (Array.isArray(p.items) ? p.items : []).slice(0, 8).map((c) => ({
			...
			props = {
				columns: String(Math.min(items.length, 4)),
				items
			};
		} else if (type === "image") {
...
*/
async function aiExtractPage(args) {
	...
	if (!blocks.length) return {
		ok: false,
		error: "هوش مصنوعی بلاکی تولید نکرد."
	};
	if (!blocks.some((b) => b.type === "hero")) blocks.unshift({
		id: newBlockId(),
		type: "hero",
		props: {
...
//#endregion
export { aiExtractPage, readPageForAi };
