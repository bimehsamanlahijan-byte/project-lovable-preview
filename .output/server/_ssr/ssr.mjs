...
function applyEnvAliases(env) {
	...
	const target = globalThis.process?.env;
	if (!target) return;
	for (const [alias, originals] of Object.entries(ALIASES)) {
		const value = originals.flatMap((original) => [target[original], source[original]]).find((candidate) => Boolean(candidate)) ?? source[alias];
		if (value && !target[alias]) target[alias] = value;
	}
}
var serverEntryPromise;
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-igD_hwpF.mjs").then((m) => m.default ?? m);
