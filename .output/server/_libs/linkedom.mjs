...
import { t as esm_exports$1 } from "./htmlparser2.mjs";
	...
	while (next !== end) {
		switch (next.nodeType) {
			...
			case 1:
				...
				next = getEnd(next);
				break;
			case 10: documentTypeAsJSON(next, json);
		}
		next = next[NEXT];
	}
...
function setOwnerDocument(node) {
	...
	switch (node.nodeType) {
		case 1:
		case 11: node.childNodes.forEach(setOwnerDocument, this);
	}
	return node;
}
//#endregion
	...
	cloneNode(deep = false) {
		...
		while (next !== prev && (deep || next.nodeType === 2)) {
			switch (next.nodeType) {
				...
				case 3:
				case 8:
				case 4: addNext(next.cloneNode(deep));
			}
			next = next[NEXT];
		}
...
//#endregion
export { parseHTML as n, esm_exports as t };
